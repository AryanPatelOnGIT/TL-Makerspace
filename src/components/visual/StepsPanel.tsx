...
export function StepsPanel({ eyebrow, title, steps, action, className, ...props }: StepsPanelProps) {
  return (
    <section className={cn('rounded-card bg-tan p-5 sm:p-6 md:p-8 text-black', className)} {...props}>
      {eyebrow && <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-black/55">{eyebrow}</p>}
      <div className="mt-3 flex flex-col gap-4 sm:gap-6 md:flex-row md:items-end md:justify-between">
      ...
      <ol className="mt-6 sm:mt-10 grid gap-3 md:grid-cols-3">
